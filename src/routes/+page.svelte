<script lang="ts">
  ...
  type DiagramNode = { id: string; label: string; x: number; y: number; tone: 'edge' | 'app' | 'data' | 'control' };
  type DiagramLink = { from: string; to: string };
  type DiagramLine = { x1: number; y1: number; x2: number; y2: number };
          ...
          <div class="border border-line bg-panel p-4">
            ...
            <div class="relative mt-4 h-[280px] overflow-hidden border border-line bg-black/25">
              ...
              {#each diagramForScenario(scenario).nodes as node}
                <button
                  class={`absolute flex min-h-10 w-[116px] -translate-x-1/2 -translate-y-1/2 items-center justify-center border px-2 text-center font-mono text-[11px] leading-4 transition hover:border-signal ${
                    node.tone === 'data'
                  ...
                  on:click={() => {
                    const target = scenario?.sections.find((section) =>
                      node.label.toLowerCase().includes(section.id)
                    );
                    if (target) changeSection(target.id);
                  }}
                >
                  {node.label}
                </button>
