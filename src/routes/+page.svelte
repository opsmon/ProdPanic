<script lang="ts">
  ...
  function diagramLines(current: Scenario): DiagramLine[] {
    ...
    return diagram.links.flatMap((link) => {
      ...
      return from && to
        ? [{ x1: from.x, y1: from.y, x2: to.x, y2: to.y, label: link.label, status: link.status }]
        : [];
    });
  }
  ...
  function nodeClasses(node: DiagramNode, compact = false) {
    const base = compact ? 'min-h-10 w-[116px] text-[11px]' : 'min-h-12 w-[142px] text-[11px] sm:w-[156px] sm:text-xs';
    const tone =
      node.tone === 'data'
